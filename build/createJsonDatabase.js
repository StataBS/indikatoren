...
files.forEach(function (filepath) {
  ...
  var fileContentsStripped = fileContents
    .slice(fileContents.indexOf("{"))
    .replace(/’/g, "'")
    .toString();
  ...
  allIndikatoren[indikator.id] = {
    id: indikator.id,
    visible: "",
    visibleInPortal: "",
    indikatorenset: indikator.kennzahlenset,
  };
  if (indikator.visible == undefined || indikator.visible == true) {
    ...
    indikator.darstellungsart =
      indikator.darstellungsart ||
      (indikator.template.includes("map") ? "Karte" : "Diagramm");
...
visibleIndikatoren.forEach((element, i, arr) => {
  ...
  if (element.visibleInPortal) {
    ...
    if (
      (element.kennzahlenset == "Umwelt" &&
        element.erlaeuterungen.indexOf(
          "Eine detaillierte Beschreibung des Indikators"
        ) == 0) ||
      element.kennzahlenset == "Test"
    ) {
...
var all = Object.keys(allIndikatoren).map(function (key) {
  return allIndikatoren[key];
});
