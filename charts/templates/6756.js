(function () {
  return {
   ...
   /* tooltip: {
      ...
      formatter: function () {
        var w;
        if ((this.point.name == 'Altersquotient') || (this.point.name == 'Jugendquotient')) {
          w = Highcharts.numberFormat((this.point.Wert * 100), 1);
        }
        else if (this.point.name == 'Baujahr&nbsp;Wohngebäude') {
          w = Highcharts.numberFormat((this.point.Wert), 0, '.', '');
        }
        else if ((this.point.name == 'Fläche&nbsp;pro&nbsp;Wohnung') || (this.point.name == 'Wohnfläche&nbsp;pro&nbsp;Kopf')) {
          w = Highcharts.numberFormat((this.point.Wert), 1) + ' m²';
        }
        else if ((this.point.name == 'Vermögenssteuer') || (this.point.name == 'Einkommenssteuer')) {
          w = Highcharts.numberFormat((this.point.Wert), 0) + ' Fr.';
        }
        else if (this.point.name == 'Arbeitsplätze&nbsp;pro&nbsp;Kopf') {
          w = Highcharts.numberFormat((this.point.Wert), 2);
        }
        else {
          w = Highcharts.numberFormat((this.point.Wert * 100), 1) + '%';
        }
        return "" +
          ...
          '<span style="color:' + this.point.color + '">\u25CF</span> ' + this.point.Wohnviertel + ': Wert <b>' + w + ' </b>Rang <b>' + this.point.y + '</b><br/>' +
          '</div>';
      }
    },*/
