...
(function (window) {
  ...
  var buildPropGetters = function (record) {
    ...
    for (selector in this.schema) {
      ...
      try {
        ...
        console.log(
          "Error while generating getter function for selector : " +
            selector +
            " NOTE: Define manually"
        );
      }
    }
  };
  ...
  var buildGetPropFn = function (field, type) {
      ...
      if (this.schema[path] == "Array") {
        if (lastArr == i) {
          ...
          accessPath =
            prefix +
            (accessPath.length
              ? ".map(function(r" + i + "){  return r" + i + accessPath + "})"
              : "");
        }
      } else {
    ...
    if (lastArr > -1) {
      accessFnBody =
        ...
        accessPath +
        ";" +
        (this.schema["path"] == "Date"
          ? "return parseDate(objs)"
          : "return objs;");
    } else {
      accessFnBody =
        "return " +
        (this.schema["path"] == "Date"
          ? "parseDate(obj" + accessPath + ");"
          : "obj" + accessPath + ";");
    }
  ...
  Q.exec = Q.toArray = function (callback) {
    ...
    if (this.criteria["or"]) {
      result = result.concat(
        execWhere.call(this, this.criteria["or"], this.records)
      );
      result = execUniq.call(this, [this.jQ.id], result);
    }

    if (this.criteria["order"]) {
      result = execOrder.call(
        this,
        this.criteria["order"],
        result || this.records
      );
    ...
    if (this.criteria["group_by"]) {
      result = execGroupBy.call(
        this,
        this.criteria["group_by"],
        result || this.records
      );
    }
  ...
  var execNear = function (opts, records) {
    ...
    each(records, function (r) {
      r._distance =
        calculateDistance(latFn(r), opts.lat, lngFn(r), opts.lng) * unit_c;
...
(function ($, window, document) {
  ...
  var setDefaultCriteriaOpts = function (criteria) {
    ...
    if (!criteria.selector) {
      if (ele.get(0).tagName == "INPUT") {
        criteria.selector =
          eleType == "checkbox" || eleType == "radio" ? ":checked" : ":input";
    ...
    if (!criteria.event) {
      criteria.event =
        eleType == "checkbox" || eleType == "radio" ? "click" : "change";
    }
  ...
  F.getSelectedValues = function (criteria, context) {
    ...
    return (
      context.execCallback("onFilterSelect", {
        criteria: criteria,
        values: vals,
      }) || vals
    );
  ...
  F.filter = function () {
    ...
    this.execCallback(
      "afterFilter",
      this.last_result,
      JsonQuery.blankClone(this.Model, this.last_result)
    );
  ...
  F.buildSearchFn = function (fields) {
    ...
    return function (text, record) {
      ...
      for (var i = 0, l = getterFns.length; i < l; i++) {
        if (
          (getterFns[i](record) + "").toLocaleUpperCase().indexOf(text) > -1
        ) {
          return true;
        }
      }
  ...
  F.searchFilter = function (records) {
    ...
    this.execCallback(
      "afterFilter",
      result,
      JsonQuery.blankClone(this.Model, result)
    );
